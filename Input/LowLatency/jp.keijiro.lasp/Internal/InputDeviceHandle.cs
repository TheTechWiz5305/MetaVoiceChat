...
namespace Lasp
{
    ...
    {
        public readonly object dataQueueLock = new();
        public readonly Queue<byte> dataQueue = new();
        ...
        public void Update(float deltaTime)
        {
            ...
            ReadOnlySpan<float> data = MemoryMarshal.Cast<byte, float>(new ReadOnlySpan<byte>(_window, 0, _windowSize));
            _audioLevels.ProcessAudioData(data);
        }
        ...
        void OpenStream()
        {
            ...
            try
            {
                ...
                _stream.SoftwareLatency = Math.Max(_device.SoftwareLatencyMin, 0.01f); // bestLatency;
          ...
          (ref SoundIO.InStreamData stream, int min, int left)
        {
            ...
            while (left > 0)
            {
                ...
                if (areas == null)
                {
                    lock (self.dataQueueLock)
                    {
                        int length = stream.BytesPerFrame * count;
                        for (int i = 0; i < length; i++)
                        {
                            self.dataQueue.Enqueue(0);
                        }
                    }
